...
if (slides.length > 0) {
    ...
    function showSlide(index) {
        ...
        if (dotsContainer) dotsContainer.innerHTML = slides.map((_, i) => `<button class="slider-dot ${i === index ? 'active' : ''}" data-index="${i}"></button>`).join('');
    }
    function nextSlide() { currentSlide = (currentSlide + 1) % slides.length; showSlide(currentSlide); }
    function prevSlide() { currentSlide = (currentSlide - 1 + slides.length) % slides.length; showSlide(currentSlide); }
    if (prevBtn) prevBtn.addEventListener('click', prevSlide);
    if (nextBtn) nextBtn.addEventListener('click', nextSlide);
    if (dotsContainer) dotsContainer.addEventListener('click', (e) => { if (e.target.classList.contains('slider-dot')) { currentSlide = parseInt(e.target.dataset.index); showSlide(currentSlide); } });
    showSlide(0);
...
function renderServicesTable(category = 'all') {
    ...
    tbody.innerHTML = filtered.map(s => `<tr><td>${s.name}</td><td>${s.unit}</td><td>от ${s.price.toLocaleString('ru-RU')} ₽</td></tr>`).join('');
    populateServiceSelects(category);
    calculatePricePage();
}
function populateServiceSelects(category = 'all') {
    const filtered = category === 'all' ? servicesData : servicesData.filter(s => s.category === category);
    const selects = [document.getElementById('service-select'), document.getElementById('home-service-select'), document.getElementById('service-select-page'), document.getElementById('service-select-form')];
    selects.forEach(select => {
        if (!select) return;
        const currentValue = select.value;
        select.innerHTML = '';
        if (select.id === 'service-select-form') { const emptyOption = document.createElement('option'); emptyOption.value = ''; emptyOption.textContent = '-- Выберите услугу --'; emptyOption.disabled = true; emptyOption.selected = true; select.appendChild(emptyOption); }
        filtered.forEach(s => { const option = document.createElement('option'); option.value = s.name; option.textContent = `${s.name} — от ${s.price} ₽/${s.unit}`; select.appendChild(option); });
        if (currentValue && [...select.options].some(o => o.value === currentValue)) select.value = currentValue;
    });
}
document.querySelectorAll('.service-tab-btn').forEach(tab => tab.addEventListener('click', () => { document.querySelectorAll('.service-tab-btn').forEach(t => t.classList.remove('active')); tab.classList.add('active'); renderServicesTable(tab.dataset.tab); }));
...
function renderPortfolio(category = 'all') {
    ...
    portfolioGrid.innerHTML = filtered.map(item => `<div class="portfolio-card"><div class="portfolio-card__image"><img src="${item.img}" alt="${item.title}" loading="lazy"><span class="portfolio-card__category">${getCategoryName(item.category)}</span></div><div class="portfolio-card__content"><h3 class="portfolio-card__title">${item.title}</h3><p class="portfolio-card__desc">${item.desc}</p><div class="portfolio-card__meta"><span><i class="fas fa-map-marker-alt"></i> Кострома</span><span><i class="fas fa-check-circle"></i> Выполнено</span></div></div></div>`).join('');
}
...
// ========== ОТКРЫТИЕ МЕНЮ ПО КЛИКУ НА МОБИЛЬНЫХ ==========
document.querySelectorAll('.nav-item.dropdown').forEach(item => {
    ...
    link.addEventListener('click', function(e) {
        if (window.innerWidth <= 768) {
            e.preventDefault();
            document.querySelectorAll('.nav-item.dropdown').forEach(other => { if (other !== item) other.classList.remove('active'); });
            item.classList.toggle('active');
        }
    });
});
document.addEventListener('click', function(e) { if (!e.target.closest('.nav-item.dropdown')) document.querySelectorAll('.nav-item.dropdown').forEach(item => item.classList.remove('active')); });
window.addEventListener('resize', function() { if (window.innerWidth > 768) document.querySelectorAll('.nav-item.dropdown').forEach(item => item.classList.remove('active')); });

// ========== ЗАКРЫТИЕ МОБИЛЬНОГО МЕНЮ ==========
document.addEventListener('click', (e) => { if (menu && menu.classList.contains('active') && !e.target.closest('.nav') && !e.target.closest('.mobile-toggle')) menu.classList.remove('active'); });
